...
Run /hedge-fund-committee-workflow and send me the INVESTMENT BRIEF.
```
...
```text
/goal the weekly INVESTMENT BRIEF has been produced via /hedge-fund-committee-workflow (regime+fed+13F+congress+
...
(`{scriptPath}`) or save as `/hedge-fund-committee-workflow` (`/workflows` → `s`). Deep-dive one ticker with
