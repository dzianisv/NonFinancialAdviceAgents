...
const NEWS_SEATS = [
  ...
  { desk: 'news-tradepress', prompt: 'You are the trade/sector-press analyst — where the NON-OBVIOUS supply-chain alpha lives (power transformers, GOES electrical steel, HBM/memory, GLP-1 CDMOs). Run /trend-stock-research for the mention-velocity baseline. For Reuters coverage, run `bun .agents/skills/read-news/scripts/read_news.ts --source googlenews --query "<theme/ticker>" --days 7` (Google News site:-restricted discovery only — treat hits as leads, never as body-quote citations; NEVER plain web_fetch a Reuters URL). Then READ free trade + business press (pv-magazine, power-eng, EE Times, trade journals — no read-news adapter exists for these niche outlets, so direct web_fetch stays correct here), IR pages, press releases, and SEC EDGAR full-text via web_fetch — these are NOT paywalled. This is the seat most likely to find the next SanDisk before the majors cover it.' + NEWS_RULES },
