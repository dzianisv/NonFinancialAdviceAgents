...
their own standalone skills (`analyse-smartmoney-13f`, `analyse-smartmoney-ptr`).
...
```bash
D="node --experimental-strip-types .agents/skills/analyse-smartmoney-13d"
...
**Ledger:** `.cache/analyse-smartmoney-13d/recommended.jsonl` — dedup scope is **ticker + filing_type + filer**.

**Roster:** `.cache/analyse-smartmoney-13d/roster.json` — known activist investors with track records.
...
- `.cache/analyse-smartmoney-13f/recommended.jsonl` — same ticker in 13F institutional filings?
...
**Sub-skill dependencies for convergence are OPTIONAL.** If `analyse-smartmoney-13f` or
`analyse-smartmoney-ptr` are unavailable, skip that convergence check and note
...
Save the final report to: **`research/analyse-smartmoney-13d-{YYYY-MM-DD}.md`**
...
A **WHICH-finder** (sibling to `analyse-smartmoney-13f`, `stocks-trend-screener`, `analyse-smartmoney-ptr`)
feeding the pipeline:

```
analyse-smartmoney-13d finds → multi-lens-quorum judges → superforecasting times
```

**Distinct from analyse-smartmoney-13f:** 13D is real-time, event-catalyst, activist-driven (micro/small-cap).
