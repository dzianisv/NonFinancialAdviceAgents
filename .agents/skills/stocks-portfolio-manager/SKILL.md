...
description: "Portfolio-agnostic equity entry-finder. Analyzes a user-supplied ticker list OR discovers stocks via current market themes (AI supply chain, robotics, energy transition, defense, fintech) discovered LIVE via web_fetch, then runs a 4-seat analyst panel per stock (fundamental / technical / narrative-macro / sentiment-positioning) in parallel subagents and outputs a concrete entry plan — entry zone, bar-close trigger, market-based stop, conviction, theme tag. Triggers: \"run the stock panel\", \"analyze these stocks: [list]\", \"find stocks in the AI supply chain theme\", \"what stocks should I look at this week\", \"run stocks-portfolio-manager\", \"find entry points for my watchlist\". Individual stocks only — ETF allocation belongs in tradfi-portfolio-manager. Educational, not advice."
license: MIT
...
metadata:
  ...
  source: "Architecture mirrors crypto-portfolio-manager (2026); seats grounded in fundamental-analysis + analyst-technical-analysis (Bernstein 2009)"
---
...
```
Run stocks-portfolio-manager on: AVGO, MRVL, VRT, CEG
```
...
```
Invoke the stocks-portfolio-manager skill.
...
## Step 0 — Seed the todo list (one row per ticker)
...
```bash
echo '{"symbol":"{TICKER}","period":"1y"}' > .agents/skills/stocks-portfolio-manager/scripts/{TICKER}.json
/Users/engineer/.venv/bin/python3 .agents/skills/stocks-portfolio-manager/scripts/fundamentals.py \
  .agents/skills/stocks-portfolio-manager/scripts/{TICKER}.json
```
...
<example>
User: "Run stocks-portfolio-manager on MRVL."
