...
Usage:
  python3 scorecard.py <fundamentals.out.json | dir_of_out_jsons> [--positions positions.csv]

positions.csv (optional, for concentration + P&L context):
  Position,Quantity,MarketValue,Unrealized_PnL   (header flexible; ticker + MV used)
"""
...
def decide(val, trend, qual, grow, weight_pct, mktcap, crypto_beta):
    """Returns (action, basis). Order matters — first match wins."""
    # 0. concentration override (risk management trumps thesis — applies even to
    #    crypto-beta: a 20%+ single EQUITY is idiosyncratic risk even if you are
    #    sector-bullish; rotate proceeds into the cleaner sector proxy, not cash)
    if weight_pct is not None and weight_pct >= 15:
        redeploy = " — rotate proceeds into BTC/cleaner crypto exposure to keep the sector bet" if crypto_beta else ""
...
CRYPTO_BETA = {"COIN","TONX","CRCL","HOOD","SOFI","IBKR","BTC","MSTR","MARA","RIOT"}
...
def load_positions(path):
    ...
    def idx(*names):
        ...
        return None
    ti, mi, pi = idx("position","ticker","symbol"), idx("marketvalue","market_value","mv"), idx("unrealized_pnl","pnl","unrealized")
    for r in rows[1:]:
...
def run_one(d, pos):
    ...
    cb = sym in CRYPTO_BETA
    vS = score_valuation(d); tS = score_trend(d); qS = score_quality(d); gS = score_growth(d)
    action, basis = decide(vS[0], tS[0], qS[0], gS[0], weight, mktcap, cb)
...
def main():
    ...
    for r in results:
        ...
        cb = " [crypto]" if r["crypto_beta"] else ""
        print(f"{r['symbol']:7} {px:>8} {wt:>5} {r['composite']:>4}  {r['action']:6}  {r['basis']}{cb}")
