...
The CIO reads all 6 analyst verdicts plus the Skeptic's challenge and makes the final call. Cannot abstain. Spawn as a subagent (`/model sonnet /effort high`). Apply with judgment, not mechanically.
...
VERDICT (believability-weighted: fundamental/narrative 2×, technical 2×; sell-side is corroborating, not primary):
