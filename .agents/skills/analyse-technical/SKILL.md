...
description: "A disciplined technical-analysis lens for TIMING LONG-TERM / SWING ENTRIES on daily/weekly charts — trend & stage (Weinstein Stage 1-4), the 200-day / 30-week trend filter, entry zones (pullbacks, base breakouts, S/R), with RSI/MACD/volume as CONFIRMATION (never standalone triggers) and multi-timeframe confluence (weekly trend gates the daily entry). Works for stocks AND crypto; computes indicators via `scripts/ta.py` (yfinance/ccxt, no Chrome) and emits a structured trend/structure/momentum/verdict read with a structure-based invalidation level. Use when the user asks \"is now a good entry\", \"what's the entry zone\", \"pullback or breakout\", \"is the trend up\", \"what stage is this in\", or \"is this a base breakout\". NOT for intraday day-trading (use `investor-bernstein-intraday`) and NOT for crypto on-chain valuation (use `analyse-onchain`). Educational, not advice; a lens, not gospel — TA has a weak/mixed empirical base, validate with backtests via `analyse-systematic-trading` before sizing."
license: MIT
