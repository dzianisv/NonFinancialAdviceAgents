...
> Source materials for the `analyse-smartmoney-darkpool` skill. The three books below provide
...
| Skill | Relationship |
|---|---|
| `analyse-smartmoney-positioning` | GEX is also a SqueezeMetrics product; shares the same data source; must be read alongside DIX for regime context |
| `analyse-smartmoney` | Parent conductor skill that routes across the full smart-money family |
| `analyse-orderflow` | Lit-market flow counterpart — tape reading, order-flow imbalance, volume-at-price; distinct from off-exchange dark-pool volume |
| `analyse-smartmoney-13f` / `analyse-smartmoney-13d` | Public SEC disclosures of institutional positions; named-party attribution that dark-pool data cannot provide |
