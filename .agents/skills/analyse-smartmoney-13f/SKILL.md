...
```bash
W="python3 .agents/skills/analyse-smartmoney-13f/watch.py"   # ledger at $THIRTEENF_LEDGER or .cache/analyse-smartmoney-13f/recommended.jsonl
...
**Ledger:** `.cache/analyse-smartmoney-13f/recommended.jsonl` — dedup scope is **ticker + quarter**. Same ticker can
resurface in a new quarter if managers show fresh action.

**Roster:** `.cache/analyse-smartmoney-13f/roster.json` — default: Burry, Buffett, Ackman, Klarman, Li Lu (with CIKs).
...
- Resolve missing CIKs via EDGAR company search; add to `.cache/analyse-smartmoney-13f/roster.json`.
