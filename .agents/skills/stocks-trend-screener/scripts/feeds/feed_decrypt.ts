/**
 * Feed: Decrypt.co
 * Unpaywalled — uses description (content:encoded no longer in feed as of Jun 2026).
 * Usage: bun .agents/scripts/feeds/feed_decrypt.ts [--db path] [--days n]
 */
...
export async function fetchDecrypt(db: Database, days: number): Promise<FeedResult> {
  const result: FeedResult = { source: "decrypt", fetched: 0, inserted: 0, enriched: 0, withinWindow: 0, errors: [] };
...
if (import.meta.main) {
  ...
  const r = await fetchDecrypt(db, days);
  db.close();
  console.log(`decrypt: fetched=${r.fetched} inserted=${r.inserted} enriched=${r.enriched} errors=${r.errors.length}`);
