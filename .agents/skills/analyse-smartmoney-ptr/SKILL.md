...
```bash
W="python3 .agents/skills/analyse-smartmoney-ptr/watch.py"
```
...
4. **Propose the NEW ones** (recommend-only): ticker, member(s), chamber, date, dollar-range, WHY. Then **record each**:
   ...
   ```
5. **Cross-check with analyse-smartmoney-13f:** ticker in a recent super-investor 13F buy → note overlap (upgrades conviction).
6. **DM the NEW proposals** only. Include: ticker, member(s), transaction date, disclosure date, dollar range, cluster size.

## Ledger management

```bash
python3 .agents/skills/analyse-smartmoney-ptr/watch.py list               # all recommended so far
python3 .agents/skills/analyse-smartmoney-ptr/watch.py list --since 2026-01-01
python3 .agents/skills/analyse-smartmoney-ptr/watch.py seen NVDA          # exit 0=seen, 1=new
```

Ledger path: `$CONGRESS_LEDGER` or `.cache/analyse-smartmoney-ptr/recommended.jsonl`
