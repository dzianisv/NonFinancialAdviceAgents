...
compatibility: >
  ...
  that can spawn subagents (Claude Code / OpenCode Task/Agent tool), the [[read-news]] pipeline (crypto
  journalism incidents — exploits/depegs/hacks), and WebSearch as fallback for incident trackers read-news
  doesn't wire (rekt.news, DefiLlama hacks dashboard, governance forums, on-chain forensics).
...
| **Risk & Incident Auditor** | Run [[read-news]] first for journalism-covered incidents (exploits, depegs, curator/oracle changes): `bun .agents/skills/read-news/scripts/read_news.ts --source theblock,coindesk,cointelegraph,decrypt --query "<protocol> exploit OR depeg OR hack OR paused withdrawals" --days 14`; WebSearch is the fallback ONLY for incident trackers read-news doesn't wire (rekt.news, DefiLlama's hacks dashboard, protocol governance-forum posts, on-chain forensics). Grade every held + candidate venue against crypto failure modes; **veto** anything with a live incident or shitty collateral. | Per-venue clean/flagged verdicts |
...
- **Incidents/news:** [[read-news]] first (Risk Auditor's job), WebSearch as fallback for niche trackers it doesn't cover — include **time-sensitive deadlines** (bridge shutdowns, migration windows) that should re-order exits.
