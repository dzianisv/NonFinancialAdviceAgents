---
name: analyse-systematic-trading
description: "Design, size, risk-manage, and validate systematic / algorithmic trading strategies through Robert Carver's framework — turn an idea into vol-standardized forecasts, combine rules with diversification multipliers, set a Kelly-anchored volatility target, size from market volatility (never account size), build a robust portfolio by handcrafting (not single-period Markowitz), and avoid over-fitting, over-trading, and over-betting by paying no more than a third of expected return in costs. Use when the user asks \"design a systematic strategy\", \"how do I size positions\", \"what vol target should I use\", \"is my backtest overfit\", \"build a trading bot in python\", \"what's the speed limit on costs\", or \"how do I validate a backtest\". Invokes Carver's ideas (man + computer + dog, EWMAC + carry, forecast scalars, Half-Kelly, handcrafting, the cost speed limit). Distilled from Robert Carver, Systematic Trading (2015). Educational, not advice; a lens, not gospel."
license: MIT
...
`analyse-technical` lens. It **operationalizes the repo mandate (GOAL.md): any strategy must be
...
- **Adjacent authors (Chan, Clenow, Kaufman) are NOT yet distilled** — cite them as pointers, not as
  this skill's grounded content. `analyse-technical` is the discretionary counterpart with a
