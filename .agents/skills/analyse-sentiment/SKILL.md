...
managed by `analyse-onchain` and extends the sentiment pillar with full metric resolution.
...
6. **Sentiment is a modulator, not a timer.** Extreme readings can persist for weeks without resolving.
   Combine with on-chain zone (`analyse-onchain` pillar 2) for higher-confidence signals.
...
| How sentiment integrates into the full four-pillar deploy decision | `analyse-onchain` (the synthesis lens) |
...
`analyse-onchain`; it does not override the liquidity governor.
...
hands off to `analyse-onchain` for integration with on-chain valuation and the liquidity governor, and
